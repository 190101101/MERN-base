...
const Bill = () => {
  ...
  useEffect(() => {
    const getBills = async () => {
      try{
        const response = await fetch(`http://localhost:5000/server/bill/${user.status == 'admin' ? 'all' : 'user/'+ user.username}`);
  ...
  return (
    <>
      ...
      <div className="px-6">
        <Table rowKey={"_id"} dataSource={billItems} columns={columns} bordered pagination={true} scroll={{
            x: 1200,
            y: 300,
          }}/>
      </div>
